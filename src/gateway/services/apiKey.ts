...
export async function trackApiKeyUsage(apiKey: string, env: Env): Promise<void> {
	...
	try {
		...
		let currentUsage = parseInt((await env.APIKI_KV.get(keyUsageKey)) || '0');
		currentUsage++; // Increment

		// Store with expiration
		await env.APIKI_KV.put(keyUsageKey, currentUsage.toString(), {
			expirationTtl: 90 * 24 * 60 * 60, // Store daily usage for 90 days
		});
	} catch (error) {
